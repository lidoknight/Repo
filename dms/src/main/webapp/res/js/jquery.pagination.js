        ...
        function drawLinks() {
            ...
            var appendItem = function(page_id, appendopts) {
                ...
                if (page_id == current_page) {
                    var lnk = $("<a class='current' href='javascript:void(0)'>" + (appendopts.text) + "</a>");
                }
                else {
            ...
            if (opts.prev_text && (current_page > 0 || opts.prev_show_always)) {
                ...
                appendopts = jQuery.extend({ text: page_id + 1, classes: "current" }, { text: opts.prev_text, classes: "prev" } || {});
                if (page_id == current_page) {
                    var lnk = $("<span class='prev'><b></b>" + (appendopts.text) + "</span>");
                }
                else {
                    var lnk = $("<a>" + (appendopts.text) + "</a>")
						.bind("click", getClickHandler(page_id))
...
//                    jQuery("<span>" + opts.ellipse_text + "</span>").appendTo(panel);
                    var lnk = $("<span>" + opts.ellipse_text + "</span>");
					if(panel.find(".pageInfo").length > 0) {
	                	panel.find(".pageInfo").before(lnk);
	                } else {
	                	panel.append(lnk);
...
//                    jQuery("<span>" + opts.ellipse_text + "</span>").appendTo(panel);
            ...
            if (opts.next_text && (current_page < np - 1 || opts.next_show_always)) {
                ...
                page_id = (current_page + 1) < 0 ? 0 : ((current_page + 1) < np ? (current_page +1) : np - 1); // Normalize page id to sane value
                appendopts = jQuery.extend({ text: page_id + 1, classes: "current" }, { text: opts.next_text, classes: "next" } || {});
                if (page_id == current_page) {
                    var lnk = $("<span class='next'><b></b>" + (appendopts.text) + "</span>");
                }
                else {
                    var lnk = $("<a>" + (appendopts.text) + "</a>")
						.bind("click", getClickHandler(page_id))
